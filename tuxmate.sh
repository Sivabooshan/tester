...
install_gnome_ext "AppIndicator Support" 'rm -rf /tmp/g-s-appindicators-build && tmpdir=$(mktemp -d) && cd "$tmpdir" && git clone https://github.com/ubuntu/gnome-shell-extension-appindicator.git && cd gnome-shell-extension-appindicator && sed -i "s/\"CHARSET=UTF-8\"/\"UTF-8\"/g" locale/it.po && meson . /tmp/g-s-appindicators-build && ninja -C /tmp/g-s-appindicators-build install && rm -rf "$tmpdir" /tmp/g-s-appindicators-build'
